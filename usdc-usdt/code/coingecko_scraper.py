...
for c in ids_to_scrape:
    ...
    if c == 'usd-coin':
        filename = 'usdc'
    filename += f"_{start_date.format('YYYY-MM-DD')}-{end_date.format('YYYY-MM-DD')}.csv"
    ...
    for c in ids_to_scrape:
        ...
        for day_start, day_end in date_range:
            ...
            try:                
                ...
                new_rows = [ { 'coin': c, 'ts': date, 'price': price, 'total_volume': volume, 'market_cap': market_cap } for date,price,volume,market_cap in zip(dates,prices,volumes,market_caps) ]
        ...
        if 'ts' in df.columns:
            try:
                df['snapped_at'] = pd.to_datetime( df['ts'], unit='ms' )
            except Exception as e:
